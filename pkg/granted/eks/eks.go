...
var proxyCommand = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		clio.Info("Ready for connections on localhost:5555")

		return listenAndProxy(ctx, yamuxStreamConnection, "5555", requestURL)
	},
}
...
func promptForclusterAndRole(ctx context.Context, accessClient accessv1alpha1connect.AccessServiceClient) (*accessv1alpha1.Entitlement, error) {
...
type ensureAccessOutput struct {
	GrantOutput *accessv1alpha1.AWSEKSProxyOutput
...
func ensureAccess(ctx context.Context, cfg *config.Context, input ensureAccessInput) (*ensureAccessOutput, error) {
	...
	if accessRequestInput.Target == "" && accessRequestInput.Role == "" {
		selectedEntitlement, err := promptForclusterAndRole(ctx, access.NewFromConfig(cfg))
		if err != nil {
			return nil, err
		}
		clio.Debugw("selected cluster and role manually", "selectedEntitlement", selectedEntitlement)
	...
	rdsOutput, ok := grantOutput.Msg.Output.(*accessv1alpha1.GetGrantOutputResponse_OutputAwsEksProxy)
	if !ok {
	...
	return &ensureAccessOutput{
		GrantOutput: rdsOutput.OutputAwsEksProxy,
		Grant:       grant.Grant,
	}, nil
}
...
func waitForSSMConnectionToProxyServer(c *cli.Context, ensuredAccess *ensureAccessOutput, serverPort, localPort string) error {
	...
	awscfg.Region = ensuredAccess.GrantOutput.EksCluster.Region
	...
	startSessionInput := ssm.StartSessionInput{
		...
		Reason: grab.Ptr(fmt.Sprintf("Session started for Granted EKS Proxy connection with Common Fate. GrantID: %s, AccessRequestID: %s", ensuredAccess.Grant.Id, ensuredAccess.Grant.AccessRequestId)),
	}
	...
	ssmLogger := &rds.SSMDebugLogger{
		Writers: []io.Writer{
			&rds.NotifyOnSubstringMatchWriter{
